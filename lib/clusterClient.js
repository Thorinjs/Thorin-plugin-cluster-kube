...
module.exports = function init(thorin, opt) {
  ...
  class ThorinClusterClient {
    ...
    async dispatch(service, action, payload = {}, _opt = {}) {
      ...
      // make sure headers are capitalized.
      const clearHeaders = {};
      Object.keys(fetchOpt.headers || {}).forEach((k) => {
        let n = k.split('-').map(q => q.charAt(0).toUpperCase() + q.substr(1)).join('-');
        clearHeaders[n] = fetchOpt.headers[k];
      });
      fetchOpt.headers = clearHeaders;
      let url;
